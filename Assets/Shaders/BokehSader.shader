Shader "Circle Shader"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float drawCircleAnimated(float2 uv, float2 center, float radius, float feather){
                float circle = pow((uv.y - center.y), 2) + pow((uv.x - center.x), 2);
                float radiusSquare = pow(radius, 2);
                if(circle < radiusSquare){
            ...
            half4 frag(VertexOutput i):COLOR 
            {
                ...
                color.a = drawCircleAnimated(i.texcoord.xy, _Center, _Radius, _Feather);
                return color;
            }
            ENDCG
        }
